...
class ModelSolutionMeta(Base):
    ...
    expected_score_max: float = Column(Float, nullable=False, default=100.0)
...
def create_model_solution(
    ...
    *,
    ...
    submission = create_model_solution_submission(
        session,
        task=task,
        participation=participation,
        digests=digests,
...
def get_or_create_model_solution_participation(session: Session):
    ...
    contest = session.query(Contest).filter(Contest.name == system_contest_name).first()
    ...
    user = session.query(User).filter(User.username == system_user_username).first()
    ...
    participation = (
        ...
        .filter(
            Participation.user_id == user.id, Participation.contest_id == contest.id
        )
        .first()
    )
