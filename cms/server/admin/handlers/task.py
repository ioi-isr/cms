        ...
        for dataset in task.datasets:
            try:
                ...
                if isinstance(score_type_obj, ScoreTypeGroup):
                    ...
                    tc_to_groups = {}
                    for group_idx, testcase_list in enumerate(targets):
                        for tc_codename in testcase_list:
                            if tc_codename not in tc_to_groups:
                                tc_to_groups[tc_codename] = []
                            tc_to_groups[tc_codename].append(group_idx)
                    testcase_groups[dataset.id] = tc_to_groups
            except Exception:
                pass
        
        self.r_params["testcase_groups"] = testcase_groups
