 ...
 */
CMS.AWSUtils.Timezone = {
    /**
     * Helper: Split a timestamp string into [mainPart, fractionalPart].
     * Example: "2026-01-01 12:00:00.123456" -> { main: "2026-01-01 12:00:00", frac: ".123456" }
     */
    getParts: function (dateStr) {
    ...
    /**
     * Get the browser's timezone name (e.g., "Asia/Jerusalem").
     */
    getName: function() {
    ...
    /**
     * Parse a datetime string in "YYYY-MM-DD HH:MM:SS" format as UTC
     * and return a Date object.
     * This ignores fractional seconds to ensure consistent Date object creation.
     */
    parseUTC: function(dateStr) {
        var p = this.getParts(dateStr);
        if (!p) return null;
        var normalized = p.main.replace(' ', 'T') + 'Z';
     ...
     * (Does not handle fractional seconds, that is handled by the caller).
     */
    ...
    /**
     * Convert local datetime string back to UTC format "YYYY-MM-DD HH:MM:SS.ffffff".
     */
    localToUTC: function(dateStr) {
        var p = this.getParts(dateStr);
        if (!p) return '';
        var localDate = new Date(p.main.replace(' ', 'T'));
        ...
        var seconds = String(localDate.getUTCSeconds()).padStart(2, '0');

        return year + '-' + month + '-' + day + ' ' + hours + ':' + minutes + ':' + seconds + p.frac;
    },
    ...
    updateTimezoneLabels: function() {
        var tzName = this.getName();
        document.querySelectorAll('.timezone-label').forEach(function(el) {
            el.textContent = tzName;
        });
    },

    /**
     * Convert datetime input fields from UTC to local for display,
     * and set up form submission handler to convert back to UTC.
     * Preserves original precision if the user doesn't edit the field.
     */
    setupDatetimeInputs: function(fieldNames, formSelector) {
        var self = this;
        ...
        fieldNames.forEach(function(fieldName) {
            ...
            if (input && input.value) {
                var p = self.getParts(input.value);
                if (p) {
                    var date = self.parseUTC(p.main);
                    if (date) {
                        var localMain = self.formatLocal(date);
                        input.value = localMain + p.frac;
                    }
                }
            }
        });

        // Guard against multiple setup calls on the same form
        if (form && !form.dataset.timezoneSetup) {
            form.dataset.timezoneSetup = 'true';
            form.addEventListener('submit', function() {
                fieldNames.forEach(function(fieldName) {
                    var input = form.querySelector('input[name="' + fieldName + '"]');
                    if (input && input.value) {
                        input.value = self.localToUTC(input.value);
                    }
                });
            });
        }
